...
struct LLMClient {
  ...
  func generate(diffs: String) async -> String {
    var prompt = "Generate an appropriate git commit message using the following diffs, describing what the commit does. Be specific. Keep responses under 100 characters Only use one emoji in the commit message, at the beginning. Do not output other text other than the commit message.\n\n```"
    prompt += diffs
    prompt += "\n```"
